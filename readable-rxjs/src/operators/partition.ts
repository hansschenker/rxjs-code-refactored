...
export function partition<T>(
  ...
  return (source: Observable<T>) => {
    const matchingValues = filter(predicate, thisArg)(source);
    const nonMatchingValues = filter(not(predicate, thisArg))(source);

    return [matchingValues, nonMatchingValues] as [Observable<T>, Observable<T>];
  };
}
