...
export function distinctUntilChanged<T, K>(
  ...
  return operate((source, subscriber) => {
    ...
    const emitIfDistinct = (value: T) => {
      const currentKey = keySelector(value);
      const isDistinct = isFirstValue || !compare(previousKey, currentKey);

      if (isDistinct) {
        // Update state before emitting to preserve reentrant behavior.
        isFirstValue = false;
    ...
    source.subscribe(
      createOperatorSubscriber(subscriber, emitIfDistinct)
    );
  });
}
