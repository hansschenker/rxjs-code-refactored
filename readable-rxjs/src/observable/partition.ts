import { not } from '../../../upstream-rxjs/src/internal/util/not';
...
export function partition<T>(
  ...
  // Each half filters its own, independent conversion of the source input:
  // values matching the predicate, and values matching the negated predicate.
  const matches = filter(predicate, thisArg)(innerFrom(source));
  const nonMatches = filter(not(predicate, thisArg))(innerFrom(source));

  return [matches, nonMatches] as [Observable<T>, Observable<T>];
}
