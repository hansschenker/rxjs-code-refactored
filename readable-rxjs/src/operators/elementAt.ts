import { ArgumentOutOfRangeError } from '../../../upstream-rxjs/src/internal/util/ArgumentOutOfRangeError';
...
export function elementAt<T, D = T>(index: number, defaultValue?: D): OperatorFunction<T, T | D> {
  ...
  const isRequestedIndex = (_: T, valueIndex: number) => valueIndex === index;
  const emitDefaultOrOutOfRangeError: OperatorFunction<T, T | D> = hasDefaultValue
    ? defaultIfEmpty<T, D>(defaultValue!)
    : throwIfEmpty<T>(() => new ArgumentOutOfRangeError());

  return (source: Observable<T>) =>
    source.pipe(
      filter(isRequestedIndex),
      take(1),
      emitDefaultOrOutOfRangeError
    );
}
