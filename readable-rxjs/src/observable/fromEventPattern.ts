...
export function fromEventPattern<T>(
  ...
  if (resultSelector) {
    // Compose the result selector on top of the selector-free version of fromEventPattern.
    return fromEventPattern<T>(addHandler, removeHandler).pipe(mapOneOrManyArgs(resultSelector));
  }

  return new Observable<T | T[]>((subscriber) => {
    // Forwards a single event argument as-is, or multiple arguments as an array.
    const handler = (...eventArgs: T[]) => subscriber.next(eventArgs.length === 1 ? eventArgs[0] : eventArgs);

    // Whatever `addHandler` returns is handed back to `removeHandler` as a
    // removal token (for APIs that unregister via a token rather than the
    // handler itself).
    const removalToken = addHandler(handler);

    // Only register teardown if `removeHandler` was actually provided.
    return isFunction(removeHandler) ? () => removeHandler(handler, removalToken) : undefined;
  });
}
