...
import { OperatorFunction, TruthyTypesOf } from '../../../upstream-rxjs/src/internal/types';
...
export function createFind<T>(
  ...
  const shouldEmitIndex = emit === 'index';

  return (source: Observable<T>, subscriber: Subscriber<any>) => {
    let index = 0;

    const testAndMaybeEmitMatch = (value: T) => {
      const currentIndex = index++;

      if (predicate.call(thisArg, value, currentIndex, source)) {
        subscriber.next(shouldEmitIndex ? currentIndex : value);
        subscriber.complete();
      }
    };

    const emitNotFound = () => {
      subscriber.next(shouldEmitIndex ? -1 : undefined);
      subscriber.complete();
    };
