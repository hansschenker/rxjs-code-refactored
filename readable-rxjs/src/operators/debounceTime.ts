...
import { MonoTypeOperatorFunction, SchedulerAction, SchedulerLike } from '../../../upstream-rxjs/src/internal/types';
...
export function debounceTime<T>(dueTime: number, scheduler: SchedulerLike = asyncScheduler): MonoTypeOperatorFunction<T> {
  return operate((source, subscriber) => {
    ...
    const rememberValueAndScheduleIfNeeded = (value: T) => {
      lastValue = value;
    ...
    source.subscribe(
      createOperatorSubscriber(
        subscriber,
        rememberValueAndScheduleIfNeeded,
        emitThenComplete,
        undefined,
        clearPendingState
      )
