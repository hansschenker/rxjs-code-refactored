...
export function repeat<T>(countOrConfig?: number | RepeatConfig): MonoTypeOperatorFunction<T> {
  ...
  return count <= 0
    ...
    : operate((source, subscriber) => {
        ...
        const createDelayNotifier = () =>
          typeof delay === 'number' ? timer(delay) : innerFrom(delay!(repeatsSoFar));
        ...
        const resubscribe = () => {
          ...
          if (delay != null) {
            const notifier = createDelayNotifier();
        ...
        const subscribeToSource = () => {
          needsSynchronousUnsubscribe = false;
