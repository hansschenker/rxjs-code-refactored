...
export function distinct<T, K>(keySelector?: (value: T) => K, flushes?: ObservableInput<any>): MonoTypeOperatorFunction<T> {
  return operate((source, subscriber) => {
    ...
    source.subscribe(
      createOperatorSubscriber(subscriber, emitIfNewKey)
    );

    if (flushes) {
      innerFrom(flushes).subscribe(createOperatorSubscriber(subscriber, clearKeys, noop));
    }
  });
}
