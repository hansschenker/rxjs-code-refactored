...
A study edition of the RxJS 7.8.x source. All 117 operators in `readable-rxjs/src/operators/`, all 34 observable creation files in `readable-rxjs/src/observable/` (including `dom/`), and 14 of the 21 scheduler files in `readable-rxjs/src/scheduler/` (the other 7 are permanent identity-preserving provider re-exports — see below) are rewrites of the upstream RxJS source into more explicit, reviewable TypeScript — while preserving public behavior exactly. A **core-classes phase is in progress**: the `readable-rxjs/src/` root, `src/scheduled/`, and `src/util/` hold scaffolding files (`Observable`, `Subscriber`, `Subscription`, the Subject family, `Scheduler`, `Notification`/`NotificationFactories`, `firstValueFrom`/`lastValueFrom`, `scheduled/*`, `util/lift`, `util/isObservable`) that are currently identity-preserving re-exports of upstream awaiting rewrite — except `src/config.ts`, which is a **permanent** re-export (see below). This is not an official RxJS distribution; it is a source-reading, refactoring, and documentation project derived from ReactiveX/RxJS (Apache-2.0, attribution must be preserved).
...
npm run test:core                                      # upstream core spec suites (spec/*-spec.ts, spec/subjects, spec/util, spec/scheduled)
npm run test:operator -- spec/operators/map-spec.ts    # one upstream spec (path relative to upstream-rxjs; works for any spec/ subdirectory)
...
2. **Runtime** — `readable-rxjs/spec/support/mocha-readable-path-mappings.js` patches Node's `Module._resolveFilename` so `rxjs`, `rxjs/operators`, `rxjs/fetch`, `rxjs/webSocket`, `rxjs/internal/operators/*`, `rxjs/internal/observable/*`, `rxjs/internal/scheduler/*`, `rxjs/internal/scheduled/*`, `rxjs/internal/util/lift`, `rxjs/internal/util/isObservable`, and the root-level core modules (`rxjs/internal/Observable`, `Subscriber`, `Subscription`, the Subject family, `Scheduler`, `Notification`, `NotificationFactories`, `firstValueFrom`, `lastValueFrom`, `config`) resolve to the readable tree during the `test:*` suites.

The tsconfig `paths` block and the mocha mapper's pattern list must stay in sync — every module remapped at runtime needs a matching compile-time path entry, and vice versa.
...
- Readable files import non-refactored internals by **relative path** into the upstream checkout, e.g. `import { operate } from '../../../upstream-rxjs/src/internal/util/lift'`. Whatever is not (yet) rewritten — most of `util/`, `Operator`, symbols, error types, `rxjs/testing` — comes from upstream.
- New areas are brought into the readable tree in two stages (the pattern used for schedulers and now core): first a **scaffolding commit** adds one-line `export * from '../../upstream-rxjs/...'` re-export files plus the tsconfig/mocha mappings (behavior-neutral, proves the routing), then individual files are rewritten in place.
...
- `readable-rxjs/src/config.ts` is likewise a **permanent identity-preserving re-export**: `config` is a mutable global settings object read at call time by both trees (Promise ctor overrides, `onUnhandledError`, `onStoppedNotification`, deprecated sync error handling), and tests mutate it through the root `rxjs` export — a readable copy would be a second, unsynchronized settings object.
...
- `readable-rxjs/src/index.ts` is the readable root index: it mirrors upstream `src/index.ts` **export-for-export**, routing operators to `./operators/*`, observable creation functions to `./observable/*`, schedulers to `./scheduler/*`, and core classes/subjects/`scheduled`/`config`/`firstValueFrom`/`lastValueFrom`/`isObservable` to the readable root files; remaining utils, error types, symbols, and types still route to upstream. Entry indexes `src/fetch/index.ts` and `src/webSocket/index.ts` mirror `rxjs/fetch` and `rxjs/webSocket`.
...
`docs/guide/` holds cross-cutting study guides (operator anatomy, flattening strategies) — the target standard for group docs is `docs/schedulers/01-async-core.md`: opening prose framing the group, per-file explanation, and "behavior-sensitive spots preserved" bullets. Group pages carry generated "See also" cross-links sourced from upstream JSDoc `@see` tags (supplemented by niklas-wortmann/rxjs-docs where upstream has none — attribution in README). The readable operator sources carry upstream's reference JSDoc verbatim; when touching an operator, keep that JSDoc in sync with upstream rather than paraphrasing it.
