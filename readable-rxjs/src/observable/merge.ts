...
import { ObservableInput, ObservableInputTuple, SchedulerLike } from '../../../upstream-rxjs/src/internal/types';
...
export function merge(...args: (ObservableInput<unknown> | number | SchedulerLike)[]): Observable<unknown> {
  // Both pops mutate `args`: a trailing scheduler (deprecated overloads) is
  // removed first, then a trailing concurrency limit, leaving only sources.
  const scheduler = popScheduler(args);
  const concurrent = popNumber(args, Infinity);
  const sources = args as ObservableInput<unknown>[];

  if (sources.length === 0) {
    // No sources provided: complete immediately.
    return EMPTY;
  }

  if (sources.length === 1) {
    // One source? There is nothing to merge with, so just return it
    // (converted to an Observable).
    return innerFrom(sources[0]);
  }

  // Merge all sources.
  return mergeAll(concurrent)(from(sources, scheduler));
}
