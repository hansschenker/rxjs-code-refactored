...
export function exhaustMap<T, R, O extends ObservableInput<any>>(
  ...
  return operate((source, subscriber) => {
    let outerIndex = 0;
    let activeInnerSubscriber: Subscriber<T> | null = null;
    ...
    source.subscribe(
      createOperatorSubscriber(
        subscriber,
        (outerValue) => {
          if (!activeInnerSubscriber) {
            activeInnerSubscriber = createOperatorSubscriber(subscriber, undefined, markInnerComplete);
            innerFrom(project(outerValue, outerIndex++)).subscribe(activeInnerSubscriber);
          }
        },
        () => {
