...
export function multicast<T, R>(
  ...
  const createSubject = isFunction(subjectOrSubjectFactory) ? subjectOrSubjectFactory : () => subjectOrSubjectFactory;

  if (isFunction(selector)) {
    return connect(selector, {
      connector: createSubject,
    });
  }

  return (source: Observable<T>) => new ConnectableObservable<any>(source, createSubject);
}
