...
import { ObservableInput } from '../../../../upstream-rxjs/src/internal/types';
  ...
  return new Observable<Response | T>((subscriber) => {
    ...
    if (outerSignal) {
      if (outerSignal.aborted) {
        // The user's signal was already aborted before we subscribed;
        // abort the internal controller immediately.
        controller.abort();
      } else {
        ...
        // Detach from the user's signal when this subscription finalizes.
    ...
    const handleError = (err: any) => {
      // The consumer sees this error; tearing down afterwards must not abort.
      abortable = false;
    ...
    fetch(input, perSubscriberInit)
      .then((response) => {
        if (selector) {
          ...
          // No selector: hand the raw Response to the consumer. Mark the fetch
          // as no longer abortable *before* notifying, so a synchronous
          // unsubscribe triggered by `next`/`complete` won't abort the body.
          abortable = false;
    ...
    return () => {
      // Only abort if the fetch is still "in flight" from the consumer's
      // perspective (no response/error delivered yet).
      if (abortable) {
        controller.abort();
      }
    };
