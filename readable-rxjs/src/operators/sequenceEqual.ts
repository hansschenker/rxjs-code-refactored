...
export function sequenceEqual<T>(
  ...
  return operate((source, subscriber) => {
    const sourceState = createState<T>();
    const compareToState = createState<T>();

    const emitAndComplete = (isEqual: boolean) => {
    ...
    const createSubscriber = (selfState: SequenceState<T>, otherState: SequenceState<T>) => {
      const sequenceEqualSubscriber = createOperatorSubscriber(
        ...
        (value: T) => {
          ...
          if (buffer.length === 0) {
            complete ? emitAndComplete(false) : selfState.buffer.push(value);
          } else {
    ...
    source.subscribe(createSubscriber(sourceState, compareToState));
    innerFrom(compareTo).subscribe(createSubscriber(compareToState, sourceState));
  });
}
