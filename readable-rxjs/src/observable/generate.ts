...
import { scheduleIterable } from '../../../upstream-rxjs/src/internal/scheduled/scheduleIterable';
...
export function generate<T, S>(
  ...
  if (arguments.length === 1) {
    ...
    if (!resultSelectorOrScheduler || isScheduler(resultSelectorOrScheduler)) {
      // The fourth argument was a scheduler (or absent), so there is no
      // result selector: emit the raw state values.
      resultSelector = identity as ResultFunc<S, T>;
  ...
  // The actual generator used to "generate" values. A missing `condition`
  // means the loop never stops on its own.
  function* generateStates(): Generator<T, void, unknown> {
  ...
  return defer(
    (scheduler
      ? // If a scheduler was provided, use `scheduleIterable` to ensure that iteration/generation
        ...
        () => scheduleIterable(generateStates(), scheduler!)
      : // Otherwise, if there's no scheduler, we can just use the generator function directly in
        // `defer` and executing it will return the generator (which is iterable).
        generateStates) as () => ObservableInput<T>
  );
}
