...
import { Unsubscribable, ObservableInput, ObservedValueOf } from '../../../upstream-rxjs/src/internal/types';
...
export function using<T extends ObservableInput<any>>(
  ...
  return new Observable<ObservedValueOf<T>>((subscriber) => {
    // Behavior note: the resource is created first, then handed to the
    // observable factory. If either factory throws, the exception is routed
    // to `subscriber.error` by the Observable's subscribe machinery.
    const resource = resourceFactory();
    const result = observableFactory(resource);
    // A falsy factory result means "no source" — behave like EMPTY.
    const source = result ? innerFrom(result) : EMPTY;
    source.subscribe(subscriber);
    // Teardown runs on completion, error, or unsubscribe, disposing the
    // resource after the source subscription is torn down.
    return () => {
