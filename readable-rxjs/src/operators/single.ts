...
import { MonoTypeOperatorFunction, OperatorFunction, TruthyTypesOf } from '../../../upstream-rxjs/src/internal/types';
import { SequenceError } from '../../../upstream-rxjs/src/internal/util/SequenceError';
import { NotFoundError } from '../../../upstream-rxjs/src/internal/util/NotFoundError';
...
export function single<T>(predicate?: (value: T, index: number, source: Observable<T>) => boolean): MonoTypeOperatorFunction<T> {
  return operate((source, subscriber) => {
    ...
    const valueMatches = (value: T) => !predicate || predicate(value, index++, source);

    const assertSingleMatch = (value: T) => {
      seenValue = true;
